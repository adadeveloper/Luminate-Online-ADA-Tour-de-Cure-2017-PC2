...
YAHOO.Convio.PC2.Component.Teamraiser = {
    ...
    loadWrapperCallback: {
        success: function(o) {
            ...
            if(typeof wrapper.isJanrainEnabled === "undefined") { // check for undefined case
                wrapper.isJanrainEnabled = false;
            }
            else {
                wrapper.isJanrainEnabled = JSON.parse(wrapper.isJanrainEnabled); // turn "boolean" string into JS booolean
            }
            ...
            Y.use("pc2-social-functions", function(Y) {
                ...
                if(wrapper.isJanrainEnabled) {
                    var socialPlaceholder = jQuery('<div id="' + wrapper.shareId + 'janrain" class="janrainSocialPlaceholder"><div>');
                    jQuery(document).ready(function(){
                        socialPlaceholder
                        .attr('data-janrain-url', wrapper.personalPageUrl)
                        .attr('data-janrain-title', wrapper.shareTitle)
                        .attr('data-janrain-message', wrapper.shareMessage);
                        socialPlaceholder.insertAfter('#sharing-sidebar-janrain');
                        janrain.social.addWidgetTo(socialPlaceholder[0]);
                    });

                    jQuery('#sharing-sidebar-janrain').removeClass('hidden-form');
                } else if(YAHOO.Convio.PC2.Component.Teamraiser.ShareContainer && wrapper.shareKey) {
