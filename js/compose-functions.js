...
function getEmailTemplateList(value) {
    ...
    for(var i=0; i<templateArray.length; i++) {
    	...
    	var messageName;
        if(data.name === 'Blank Message')
            messageName = YAHOO.Convio.PC2.Component.Content.getMsgCatValue("blank_message_header");
        else
...
function getContactString(contact) {
    ...
    if(YAHOO.lang.isString(contact.firstName)) {
        var fName = YAHOO.Convio.PC2.Utils.htmlUnescape(contact.firstName.replace(/(,|<|>)/g,""));
    ...
    if(YAHOO.lang.isString(contact.lastName)) {
        ...
        var lName = YAHOO.Convio.PC2.Utils.htmlUnescape(contact.lastName.replace(/(,|<|>)/g,""));
